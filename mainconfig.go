...
func LoadConfig() *Config {
	...
	fmt.Println("✅ [Main] 成功載入環境變數 envfile (CWD)")
	GlobalName = os.Getenv("BotSocketNam") // 外出使用 web socket 時使用的名稱，可自行更換
	CoreSystemPrompt := "你是一個專業的助手。"
	/*
	...
	*/
	cfg := &Config{
		// 從環境變數讀取，若無則使用後方的預設値
		Model:        getEnv("DEFAULT_MODEL", "gemma4:latest"),
