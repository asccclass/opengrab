...
import (
	...
	"github.com/asccclass/opengrab/internal/agent"
...
func runChat(cmd *cobra.Command, args []string) {
	...
	*/
	// -------------------------------------------------------------
	// 5. 初始化 Agent (internal/agent)
	// -------------------------------------------------------------
	mainAgent := agent.NewAgent(providerName, modelName, systemPrompt) // , sess, registry, logger)
	if mainAgent == nil {
		fmt.Printf("❌ [Main] Failed to create agent\n")
		return
	}
	/*
	...
	*/
	mainAgent.OnModelMessageComplete = func(content string) {
		// 清除行 (如果是思考中...)
		fmt.Print("\r\033[K")
		...
		if content != "" {
			...
			fmt.Println("❌ [Main] No content received from agent")
		}
	}
	/*
	...
	*/
	for {
		...
		*/
		...
		// 交給 Agent 處理
		_, err := mainAgent.Chat(input, nil) // CLI 暫不使用 Realtime stream raw text，而是依賴 Callbacks 渲染 Markdown
		if err != nil {
			fmt.Printf("❌ 錯誤: %v\n", err)
		}
		/*
